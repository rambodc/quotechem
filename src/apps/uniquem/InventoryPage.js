...
export default function InventoryPage({ data, lookups, mutate }) {
  ...
  {drawer?.type === 'warehouse' ? <WarehouseDrawer warehouse={drawer.warehouse} busy={busy} onClose={() => setDrawer(null)} onSave={(payload) => run('saveUniquemWarehouseV2', payload)} onDelete={() => window.confirm('Permanently delete this unused warehouse?') && run('deleteUniquemWarehouse', { warehouseId: drawer.warehouse.warehouseId })} /> : null}
  {drawer?.type === 'adjust' ? <AdjustmentDrawer batch={drawer.batch} product={lookups.products.get(drawer.batch.productId)} busy={busy} onClose={() => setDrawer(null)} onSave={(payload) => run('adjustUniquemInventoryV2', payload)} /> : null}
