...
export default function QuoteChemInbox() {
  ...
  if (selected) {
    ...
    return <section className="qci-page qci-detail">
      ...
      <div className="qci-summary">
        <article><small>Status</small><strong>{conversation.status === 'submitted' ? 'Submitted request' : conversation.status === 'abandoned' ? 'Abandoned conversation' : 'Unfinished conversation'}</strong></article>
  ...
  return <section className="qci-page">
    ...
    <div className="qci-filters"><label><FiSearch /><input aria-label="Search requests" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search request, company, email, need…" /></label><select aria-label="Filter submission status" value={status} onChange={(event) => setStatus(event.target.value)}><option value="">All conversations</option><option value="submitted">Submitted requests</option><option value="active">Active conversations</option><option value="ready">Ready for contact</option><option value="abandoned">Abandoned conversations</option></select><select aria-label="Filter sourcing stage" value={stage} onChange={(event) => setStage(event.target.value)}><option value="">All sourcing stages</option>{STAGES.map(([id, label]) => <option key={id} value={id}>{label}</option>)}</select></div>
    {error ? <p className="qci-error">{error}</p> : null}
    <div className="qci-list">{items.map((item) => <button type="button" key={item.conversationId} onClick={() => open(item.conversationId)}><span className={`qci-status ${item.status}`}>{item.status === 'submitted' ? item.requestId : item.status === 'abandoned' ? 'Abandoned' : 'Conversation'}</span><strong>{item.contact?.company || item.guidedContext?.needLabel || 'Open inquiry'}</strong><small>{item.contact?.name || item.contact?.email || 'Anonymous visitor'}</small><em>{item.status === 'submitted' ? stageLabel(item.sourcingStage) : item.status === 'abandoned' ? 'Abandoned draft' : 'Not submitted'} · {shownDate(item.updatedAt)}</em></button>)}</div>
