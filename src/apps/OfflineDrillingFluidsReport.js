...
export default function OfflineDrillingFluidsReport() {
  ...
  const uploadReport = async (report) => {
    ...
    const owner = readPreparedDrillingUser() || preparedUser;
    if (!activeUser || !owner?.uid) throw new Error('Sign in to upload.');

    await setDoc(
      doc(db, 'drillingFluidReports', report.localId),
      {
        createdBy: owner.uid,
        createdByEmail: owner.email || activeUser.email || '',
  ...
  const uploadPending = async () => {
    ...
    if (!auth.currentUser || !readPreparedDrillingUser()?.uid) {
      setError('Sign in to upload. Your local reports will stay saved on this device.');
      return;
    }
  ...
  return (
    <main className="drilling-report-page offline-standalone" aria-labelledby="drilling-report-title">
      ...
      {!preparedUser?.uid ? (
        <p className="drilling-message error">
          Open this report from the QuoteChem portal once while online before field use. You can still save local drafts, but upload will ask you to sign in.
        </p>
      ) : null}
      ...
      <div className="drilling-layout">
        <form className="drilling-form" onSubmit={(event) => event.preventDefault()}>
          ...
          <div className="drilling-actions">
            <Link to="/signin" className="secondary sign-in-link">
              Sign in
            </Link>
          ...
          {reports.map((report) => (
            <article key={report.localId} className={`local-report-card ${report.status}`}>
              <div>
                ...
                <span>
                  {report.payload?.reportDate || 'No date'} · {statusLabel(report.status)}
                </span>
              </div>
