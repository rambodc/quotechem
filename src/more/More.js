...
import { FiChevronRight, FiLogOut, FiMail, FiUser } from 'react-icons/fi';
...
export default function More() {
  ...
  const [testEmail, setTestEmail] = useState(appUser?.email || '');
  ...
  const sendTest = async (template) => {
    ...
    try {
      const data = await postJson('sendTestEmail', { email: testEmail, template });
  ...
  return (
    <section className="more-page">
      ...
      <article className="more-profile">
        <h3 style={{ margin: '0 0 10px' }}>Email Test Buttons</h3>
